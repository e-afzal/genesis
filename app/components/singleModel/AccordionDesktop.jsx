...
const AccordionDesktop = ({ modelName, styles, data }) => {
  const [currentImage, setCurrentImage] = useState(`/images/car_models${data[0].image}`);

  const handleImageChange = (val) => {
    const accessory = data.filter(accessory => accessory.title === val);
    setCurrentImage(`/images/car_models${accessory[0].image}`);
  };

  return (
    <div className={styles.tab_container_desktop}>
      <div className={styles.tab_content}>
        <h2 className={styles.accessories_title}>{modelName} exclusive accessories</h2>
        <Accordion.Root
        ...
        >
          {data.map((each, index) => (
            <Accordion.Item key={index} value={each.title}>
              <Accordion.Header>
                <Accordion.Trigger className={styles.accordion_trigger}>{each.title}</Accordion.Trigger>
              </Accordion.Header>
